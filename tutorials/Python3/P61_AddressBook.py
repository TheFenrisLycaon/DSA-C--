...
class AddressBook(object):
    ...
    def __str__(self):
        return "[Name: {0} | Address: {1} | Email: {2} | Phone: {3}]".format(
            self.name, self.address, self.email, self.phone
        )
    ...
    def getDetailsFromUser(self):
        try:
            self.contacts["Name"] = str(eval(input("Enter Contact's Full Name: ")))
            self.contacts["Address"] = str(eval(input("Enter Contact's Address: ")))
            self.contacts["Email"] = str(eval(input("Enter Contact's Email Address: ")))
            self.contacts["Phone"] = int(eval(input("Enter Contact's Phone Number: ")))
    ...
    def searchContacts(self):
        if os.path.exists(self.filename) and os.path.getsize(self.filename) > 0:
            ...
            myAddressBook.close()
            try:
                contactToSearch = eval(input("Enter the name of the contact to search: "))
                counter = 0
                for contact in list(data.values()):
    ...
    def modifyContacts(self):
        if os.path.exists(self.filename) and os.path.getsize(self.filename) > 0:
            ...
            myAddressBook.close()
            try:
                contactToModify = eval(input(
                    "Enter the name of the contact to modify (Only enter full name): "
                ))
                ...
                option = int(
                    eval(input(
                        "1. To modify name, 2. To modify address, 3. To modify email, 4. To modify phone: "
                    ))
                )
                if option == 1:
                    contact["Name"] = eval(input("Enter Name to modify: "))
                    del data[contactToModify]
                    data[contact["Name"]] = contact
                    print("Successful")
                elif option == 2:
                    contact["Address"] = eval(input("Enter Address to modify: "))
                    del data[contactToModify]
                    data[contactToModify] = contact
                    print("Successful")
                elif option == 3:
                    contact["Email"] = eval(input("Enter Email to modify: "))
                    del data[contactToModify]
                    data[contactToModify] = contact
                    print("Successful")
                elif option == 4:
                    contact["Phone"] = eval(input("Enter Phone to modify: "))
                    del data[contactToModify]
                    data[contactToModify] = contact
                    print("Successful")
                else:
...
if __name__ == "__main__":
    ...
    while True:
        choice = int(eval(input("Enter your choice: ")))
        if choice == 1:
            myBook.addContacts()
        elif choice == 2:
